      ...
      <div className="mb-4 max-h-96 overflow-y-scroll scrollbar-thin scrollbar-thumb-primary scrollbar-track-secondary-light scrollbar-track-rounded-full">
        {trendingPosts.length &&
          trendingPosts.map((element: any, index: number) => {
            ...
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.3, type: "spring", stiffness: 110, delay: (index % 4) * 0.4 + 0.6 }}
            className="mb-2">{element.caption}</motion.p>

                <motion.p initial={{ opacity: 0, x: 50 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.3, type: "spring", stiffness: 110, delay: (index % 4) * 0.6 + 0.8 }}
            className="text-sm text-secondary-light dark:text-primary-light">Read more...</motion.p>
              </Link>
            );
          })}
      </div>
